...
export default function ActivityPage() {
  ...
  const [regStatus, setRegStatus] = useState('idle') // idle|loading|success|error|full
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setActivity(data)
        if (user) {
          const c = await axios.get('/api/children')
          setChildren(c.data)
        }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setRegMsg('Inscription enregistrée ! Vous recevrez un email de confirmation.')
    } catch (e) {
      setRegStatus('error')
                    ...
                    <>
                      {children.length === 0 ? (
                        ...
                        <>
                          ...
                          >
                            {regStatus === 'loading' ? '⏳ Inscription...' : '🎿 S\'inscrire à cette activité'}
                          </button>
                        </>
                      )}
                    </>
