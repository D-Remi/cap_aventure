...
const EMPTY_IND = { date:'', heure_debut:'09:00', heure_fin:'17:00', motif:'' }
...
export default function AdminPlanning() {
  ...
  useEffect(() => { fetchSlots(); fetchIndispos() }, [])

  const fetchSlots    = () => axios.get('/api/slots?all=true').then(r => setSlots(r.data)).catch(() => {})
  const fetchIndispos = () => axios.get('/api/indisponibilites').then(r => setIndispos(r.data)).catch(() => {})

  const set    = k => e => setForm(f => ({ ...f, [k]: e.target.type === 'checkbox' ? e.target.checked : e.target.value }))
  const setInd = k => e => setIndForm(f => ({ ...f, [k]: e.target.value }))

  const openCreate = () => { setForm(EMPTY); setModal('create') }
  const openEdit   = s  => {
    setForm({ ...s, heure_debut: s.heure_debut?.slice(0,5)||'09:00', heure_fin: s.heure_fin?.slice(0,5)||'17:30', tarif: String(s.tarif||25) })
    setModal({ id: s.id })
  }
  ...
  const saveSlot = async () => {
    ...
    setSaving(true)
    try {
      const payload = { ...form, places_max: Math.min(Number(form.places_max), 3), tarif: parseFloat(form.tarif) }
      if (modal === 'create') await axios.post('/api/slots', payload)
      else                    await axios.put(`/api/slots/${modal.id}`, payload)
      setModal(null); fetchSlots(); toast.success('Créneau enregistré')
  ...
  const ii = (label, key, type = 'text', ph = '') => (
    <div>
      <label style={{ fontSize:'.78rem', fontWeight:700, color:'var(--nuit)', display:'block', marginBottom:4, textTransform:'uppercase' }}>{label}</label>
      <input type={type} value={indForm[key]} onChange={setInd(key)} placeholder={ph}
        style={{ width:'100%', padding:'.5rem', border:'1.5px solid var(--sable-dark)', borderRadius:8, fontFamily:'inherit', fontSize:'.9rem' }}/>
    </div>
  )
          ...
          <div className="admin-table-wrap">
            {slots.length === 0 ? (
              ...
              <table className="admin-table">
                <thead><tr><th>Date</th><th>Créneau</th><th>Type</th><th>Places</th><th>Tarif</th><th>Statut</th><th>Actions</th></tr></thead>
                <tbody>
                  {slots.map(s => (
                    <tr key={s.id}>
                      ...
                      <td>
                        <div style={{ display:'flex', alignItems:'center', gap:'.4rem' }}>
                          <div style={{ width:52, height:7, background:'#e0e0e0', borderRadius:4, overflow:'hidden' }}>
                            <div style={{ width:`${(s.places_prises/s.places_max)*100}%`, height:'100%', background: s.places_prises>=s.places_max?'#ef4444':'var(--sauge)', borderRadius:4 }}/>
                          </div>
                      ...
                      <td>
                        <span style={{
                          background:{ouvert:'#e8f5e9',complet:'#fee2e2',annule:'#f3f4f6',passe:'#f9fafb'}[s.statut]||'#f3f4f6',
                          color:{ouvert:'#2e7d32',complet:'#991b1b',annule:'#6b7280',passe:'#9ca3af'}[s.statut]||'#6b7280',
          ...
          <div className="admin-modal-overlay" onClick={() => !saving && setModal(null)}>
            <div className="admin-modal" style={{ maxWidth:560 }} onClick={e => e.stopPropagation()}>
              ...
              <div className="admin-modal__actions">
                ...
                <button className="btn-primary" onClick={saveSlot} disabled={saving || !form.date}>{saving ? 'Enregistrement…' : 'Enregistrer'}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="admin-modal-overlay" onClick={() => setIndModal(false)}>
            <div className="admin-modal" style={{ maxWidth:400 }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'.85rem' }}>
                {ii('Date *', 'date', 'date')}
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.75rem' }}>
                  {ii('Début', 'heure_debut', 'time')}
                  {ii('Fin',   'heure_fin',   'time')}
                </div>
                {ii('Motif', 'motif', 'text', 'Ex: RDV médecin, Vacances...')}
              </div>
              <div className="admin-modal__actions">
