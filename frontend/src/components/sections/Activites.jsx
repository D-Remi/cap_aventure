...
export default function Activites() {
  ...
  const applyTranslate = useCallback((idx) => {
    ...
    trackRef.current.style.transform = `translateX(${-idx * (cards[0].offsetWidth + 16)}px)`
  }, [])
  ...
  const go = useCallback((idx) => {
    const c = Math.max(0, Math.min(idx, steps))
    setCurrent(c); applyTranslate(c); resetAuto()
  }, [steps, applyTranslate, resetAuto])

  useEffect(() => { resetAuto(); return () => clearInterval(autoRef.current) }, [resetAuto])

  useEffect(() => {
    const obs = new ResizeObserver(e => setVisible(getVisible(e[0].contentRect.width)))
        ...
        <div className="carousel-outer">
          <div className="carousel-wrap" ref={wrapRef}>
            <div className="carousel-track" ref={trackRef}
              onTouchStart={e => { startXRef.current = e.touches[0].clientX; clearInterval(autoRef.current) }}
              onTouchEnd={e => { const dx = e.changedTouches[0].clientX - startXRef.current; if (Math.abs(dx) > 40) go(dx < 0 ? current + 1 : current - 1); else resetAuto() }}
              onMouseDown={e => { startXRef.current = e.clientX; isDragging.current = true; clearInterval(autoRef.current) }}
              onMouseUp={e => { if (!isDragging.current) return; const dx = e.clientX - startXRef.current; if (Math.abs(dx) > 40) go(dx < 0 ? current + 1 : current - 1); else resetAuto(); isDragging.current = false }}
